...
#
# Depends on stacked PRs:
#   [1/N] multiturn agentic trajectory (image_gen_tool_agent + Hydra agentic_image_gen)
#   [2/N] UniCoT builder (build_unicot_agentic_rl)
#   [3/N] RPCO multidim scorer (agentic_multidim_reward + naive reward manager)
#
...
# Builder bakes RPCO_W_* into parquet ground_truth (PR #411). Accept the
# anchor's ``tool_call`` spelling as an alias for ``tool``.
...
# Qwen3-VL is not an OmniModelBase architecture, so use verl main_ppo (not
# main_omni). Inject agentic_image_gen via ``+`` and bind tools through
# OmniAgentLoopManager (PR #409). V1 main_ppo passes TensorDict into
# generate_sequences; the manager switches to TransferQueue workers when
# transfer_queue.enable is on.
