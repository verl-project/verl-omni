...
# Fail fast on all batch/rank divisibility constraints touched by rollout,
# old-logprob/ref-logprob, and actor update.  The trainer multiplies
# actor.ppo_mini_batch_size by rollout.n before dispatching actor update.
# PPO_MINI_BATCH_SIZE is prompt-level; keep it small enough that
# ACTOR_UPDATE_MINI_BATCH_PER_GPU fits memory.
...
require_divisible ROLLOUT_BATCH_PER_GPU "$ROLLOUT_BATCH_PER_GPU" ACTOR_UPDATE_MINI_BATCH_PER_GPU "$ACTOR_UPDATE_MINI_BATCH_PER_GPU" "engine_workers.train_mini_batch local tensordict batch must divide local mini-batch; otherwise assertion like '2 % 4 != 0'"
...
# Diffusion infer_batch uses prepare_micro_batches with force_group_size=1 unless
# a caller explicitly injects force_group_size into the TensorDict.  The current
# old/ref logprob path does not inject rollout.n, so these micro-batches divide
# local rollout samples directly (same convention as Wan2.2/Qwen image examples).
