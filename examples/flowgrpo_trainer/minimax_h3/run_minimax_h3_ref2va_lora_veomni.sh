#!/usr/bin/env bash
# MiniMax H3 Ref2VA LoRA FlowGRPO with the VeOmni actor engine.
set -euxo pipefail
...
ACTOR_CONFIG_PATH=${ACTOR_CONFIG_PATH:-$MODEL_PATH/Ref2VA/transformer}
NUM_GPUS=${NUM_GPUS:-8}
...
if [[ ! -d "$MODEL_PATH/Ref2VA/transformer" ]]; then
    echo "MODEL_PATH must contain the fused rollout checkpoint at Ref2VA/transformer/." >&2
    exit 1
fi
