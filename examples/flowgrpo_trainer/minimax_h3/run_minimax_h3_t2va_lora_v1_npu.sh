#!/usr/bin/env bash
# MiniMax H3 T2VA LoRA FlowGRPO on Ascend NPU (V1 sync trainer).
# Training and NPU settings follow run_minimax_h3_t2va_lora_npu.sh.
set -x
...
ACTOR_SP=${ACTOR_SP:-1}
if (( NUM_GPUS <= 0 || ACTOR_SP <= 0 || NUM_GPUS % ACTOR_SP != 0 )); then
...
python3 -m verl_omni.trainer.main_diffusion_v1 \
    ...
    trainer.experiment_name=minimax_h3_t2va_lora_v1_npu \
