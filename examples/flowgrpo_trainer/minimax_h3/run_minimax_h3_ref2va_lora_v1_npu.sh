#!/usr/bin/env bash
# MiniMax H3 REF2VA LoRA FlowGRPO on Ascend NPU (V1 sync trainer).
# Task settings follow the GPU recipe; NPU settings follow the T2VA NPU recipe.
...
SDE_WINDOW_END=${SDE_WINDOW_END:-8}
...
ACTOR_ATTN_BACKEND=${ACTOR_ATTN_BACKEND:-_native_npu}
...
if (( N_GPUS <= 0 || ROLLOUT_TP <= 0 || N_GPUS % ROLLOUT_TP != 0 )); then
    echo "N_GPUS must be a positive multiple of ROLLOUT_TP" >&2
    exit 1
fi
...
python3 -m verl_omni.trainer.main_diffusion_v1 \
    ...
    trainer.experiment_name=minimax_h3_ref2va_lora_v1_npu \
