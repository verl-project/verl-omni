...
"""Convert MiniMax H3 T2VA text-prompt splits to verl-omni parquet files.

Reads ``train.txt``/``test.txt`` (one prompt per line) or
``train.jsonl``/``test.jsonl`` (``prompt``/``text``/``caption`` fields) and
writes prompt-only parquet consumed by ``run_minimax_h3_t2va_lora.sh``.
...
def main() -> None:
    ...
    print(f"Wrote {len(train)} training and {len(validation)} validation rows to {output_dir}")
