...
class OmniRewardLoopManager(RewardLoopManager):
    ...
    Each reward worker is given access to **all** devices in the visible pool so that multiple
    reward functions configured with different device indices (e.g. ``npu:0`` and ``npu:1``)
    can coexist within a single worker process.  When the launcher restricts visible devices
    via the platform's visible-devices env var (e.g. ``ASCEND_RT_VISIBLE_DEVICES=8,9,...,15``),
    all workers inherit that same subset, keeping reward workers co-located with actor/rollout
    workers on the user-selected devices rather than spilling onto die 0..N.
    """
    ...
    def _init_reward_loop_workers(self):
        ...
        for i in range(num_workers):
            ...
            if all_cpu:
                ...
                # Expose every device in the visible pool so that reward functions
                # configured with different device indices (e.g. npu:0, npu:1) can
                # all access their target device within a single worker process.
                env_vars = {env_var: ",".join(str(d) for d in visible_pool), **noset_vars}
