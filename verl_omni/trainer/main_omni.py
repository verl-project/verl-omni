...
* **Offline actor-only trainers** (DPO and SFT): uses ``RayTrainerTaskRunner``
  with the unified model engine.
"""
...
from verl_omni.trainer.omni.ray_omni_trainer import SFTRayTrainer
...
def get_ray_trainer_cls(config):
    ...
    raise ValueError(
        f"Unsupported trainer_type {trainer_type!r}. Expected one of: 'policy_gradient', 'direct_preference', 'sft'."
    )
...
class RayTrainerTaskRunner:
    ...
    def add_reward_model_resource_pool(self, config):
        ...
        if config.algorithm.sample_source == "online" and config.reward.reward_model.enable:
            if config.reward.reward_model.enable_resource_pool:
    ...
    def run(self, config):
        ...
        if "val_sampler" in inspect.signature(trainer_cls.__init__).parameters:
            trainer_kwargs["val_sampler"] = val_sampler
...
def uses_v1_trainer(config) -> bool:
    """Return True unless the config explicitly selects an offline actor-only trainer."""
    ...
    return not (sample_source == "offline" and trainer_type in {"direct_preference", "sft"})
