...
class vLLMOmniHttpServer(vLLMHttpServer):
    ...
    def _process_output(self, final_res, params, sampling_params: dict[str, Any]):
        ...
        # Forward every custom_output key (all_latents, all_timesteps,
        # prompt_embeds, image_latents, img_shapes, latents_clean, etc.) to
        # the training micro_batch. Upstream introduced a generic
        # `_maybe_unbatch` helper that subsumes the prior hardcoded field
        # list — including the condition-image fields (image_latents,
        # img_shapes) that I2I rollout adapters emit so the training adapter
        # can re-concatenate ``[hidden_states, image_latents]`` exactly as
        # rollout did (otherwise the importance ratio is computed under
        # mismatched transition densities and reward cannot climb).
