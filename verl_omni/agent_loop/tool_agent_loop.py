...
"""The image-gen agent loop: one rollout of prompt → tool calls → ``Done.``.

Builds on verl's stock ``ToolAgentLoop`` and adds two training aids: the first
``generate_image`` call can be written for the model (curriculum), and a reflection
turn is inserted after each judge verdict. Text written for the model is trained on;
inserted reflection is context only. The final ``Done.`` is generated by the model.
"""
...
class ImageGenToolAgentLoop(ToolAgentLoop):
    """Agent loop for image tasks: prompt → tool calls → ``Done.``.

    Runs the multi-turn protocol and adds the two training aids described in this
    module's docstring (write the first tool call for the model; insert a reflection
    turn after each judge verdict).
    """
    ...
    async def _replace_last_assistant_with_tool_call(
        ...
        """Replace the last sampled assistant text with tool-call text written for the model.
    ...
    async def _handle_generating_state(
        ...
        """Write missing generate/judge tool calls for the model during early training."""
