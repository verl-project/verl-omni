...
export RUN_ID="${RUN_ID:-pruned_vllm_omni_tp2_replica2_standalone_logprob_$(date +%Y%m%d_%H%M%S)}"
...
export STAGE_CONFIG="${STAGE_CONFIG:-${SCRIPT_DIR}/qwen3_omni_thinker_only_tp2_dp2_pruned_async_raw_logprobs.yaml}"
...
echo "[info] pruned TP2 x replica2 standalone vLLM-Omni debug JSONL prefix: ${VERL_OMNI_VLLM_LOGPROB_DEBUG_JSONL}"
