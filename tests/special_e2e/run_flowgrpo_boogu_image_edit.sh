#!/usr/bin/env bash
# FlowGRPO Boogu-Image-Edit (TI2I) e2e smoke test, vllm_omni rollout.
#
# Exercises the FlowGRPO pipeline with the Boogu editing path:
#   parquet load (with condition image) -> vllm_omni rollout
#   (reference latents via the ref_image_hidden_states refiner path) ->
...
#   (see tests/special_e2e/build_boogu_image_tiny_random.py; the Base and
#   Edit checkpoints share the pipeline architecture, so one tiny checkpoint
#   serves both smoke tests).
set -euo pipefail
...
echo "FlowGRPO Boogu-Image-Edit e2e test passed (training completed successfully)."
