...
# 32-GPU raw-logprobs route debug with n=1 while preserving the baseline worker
# concurrency.  This separates repeated-response packing effects from general
# async rollout pressure.

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"

export RUN_ID="${RUN_ID:-full32_vllm_omni_n1_parallel_route_debug_$(date +%Y%m%d_%H%M%S)}"
export EXP_NAME="${EXP_NAME:-qwen3_omni_megatron_full_32gpu_vllm_omni_n1_parallel_route_debug_${RUN_ID}}"
export PROFILE_LABEL="${PROFILE_LABEL:-32-GPU fullmodel vLLM-Omni n=1 parallel raw route debug probe}"
...
export OMNI_LB_POLICY="${OMNI_LB_POLICY:-random}"
