...
if [[ $# -lt 1 && -z "${DUMP_JSONL:-}" ]]; then
  echo "usage: $0 /path/to/rollout_corr_debug.jsonl" >&2
...
RUN_ID="${RUN_ID:-vllm_score_rollout_corr_$(date +%Y%m%d_%H%M%S)}"
...
OUTPUT_FILE="${OUTPUT_FILE:-${OUT_DIR}/${RUN_ID}.vllm_score.jsonl}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/${RUN_ID}.log}"

python3 "${SCRIPT_DIR}/qwen3_omni_vllm_score_rollout_corr_dump.py" \
