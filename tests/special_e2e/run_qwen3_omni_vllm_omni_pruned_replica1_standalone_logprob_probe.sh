...
export RUN_ID="${RUN_ID:-pruned_vllm_omni_replica1_standalone_logprob_$(date +%Y%m%d_%H%M%S)}"
...
export ROW_INDICES="${ROW_INDICES:-0,1,2,3}"
...
echo "[info] replica1 standalone vLLM-Omni debug JSONL prefix: ${VERL_OMNI_VLLM_LOGPROB_DEBUG_JSONL}"
