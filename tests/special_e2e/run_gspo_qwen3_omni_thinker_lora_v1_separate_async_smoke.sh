#!/usr/bin/env bash
# Qwen3-Omni Thinker GSPO + LoRA separate-async V1 smoke test (minimal runtime).
#
# Exercises trainer_mode=omni_separate_async: 1 GPU for the FSDP trainer plus 1 GPU
...
# ── Run separate-async V1 training (3 steps, LoRA, GSPO, standalone rollout) ───
python3 -m verl_omni.trainer.main_omni \
    ...
    trainer.experiment_name=gspo-qwen3-omni-thinker-lora-separate-async-e2e-v1 \
...
echo "Qwen3-Omni Thinker GSPO+LoRA separate-async e2e V1 smoke test passed (training completed successfully)."
