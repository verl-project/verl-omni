...
def test_worker_join_retry_is_bounded_and_refreshes_negotiated_head_port():
    script = SCRIPT.read_text(encoding="utf-8")
    start = script.index('echo "[info] Starting Ray worker rank', script.index("start_ray_cluster() {"))
    end = script.index('if [[ "${RAY_WORKER_EXIT_ON_MASTER_DONE}" == "1" ]]', start)
    worker_join = script[start:end]

    assert 'refresh_ray_head_address_from_file "${RAY_HEAD_PORT_FILE}"' in worker_join
    assert '"${RAY_WORKER_JOIN_ATTEMPT_TIMEOUT}" \\' in worker_join
    assert "        ray start \\" in worker_join
    assert 'ray stop --force >/dev/null 2>&1 || true' in worker_join
    assert worker_join.index("refresh_ray_head_address_from_file") < worker_join.index("ray start")


def test_config_only_rejects_nonpositive_worker_join_timeout(tmp_path: Path):
    output = _run_config_only_failure(tmp_path, RAY_WORKER_JOIN_ATTEMPT_TIMEOUT="0")

    assert "RAY_WORKER_JOIN_ATTEMPT_TIMEOUT must be a positive integer, got 0" in output
...
def test_config_only_shifts_ray_worker_block_when_preferred_block_is_busy(tmp_path: Path):
    ...
    assert "worker_preferred_block=" in second_output
    assert "worker_local_probe=1" in second_output
