...
choice_reward. patch_minicpm_answer_tags demotes the two tokens in the
...
def test_patch_minicpm_answer_tags_restores_reward():
    tokenizer = _minicpm_style_tokenizer()
    answer_id = tokenizer.convert_tokens_to_ids("<answer>")
    think_id = tokenizer.convert_tokens_to_ids("<think>")

    assert patch_minicpm_answer_tags(tokenizer) is True
...
def test_keep_answer_tags_is_noop_when_already_plain():
    ...
    assert patch_minicpm_answer_tags(tokenizer) is False  # second call: nothing to demote
    ...
    assert patch_minicpm_answer_tags(plain) is False
...
def test_keep_answer_tags_raises_when_unfixable():
    ...
    assert patch_minicpm_answer_tags(_NoBackend()) is False
    ...
    with pytest.raises(RuntimeError, match="no fast backend"):
        patch_minicpm_answer_tags(_SpecialNoBackend())
