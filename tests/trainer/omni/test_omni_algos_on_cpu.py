...
class TestOmniLossRegistry:
    def test_builtin_dpo_registered(self, omni_algos):
        ...
        assert "omni_sft" in omni_algos.OMNI_LOSS_REGISTRY
    ...
    def test_get_existing_loss_fn(self, omni_algos):
        ...
        sft_fn = omni_algos.get_omni_loss_fn("omni_sft")
        assert isinstance(sft_fn, omni_algos.OmniSFTLoss)
...
class TestOmniSFTLossCallable:
    def test_callable_combines_text_and_image_terms(self, omni_algos, OmniLossConfig):
        loss_fn = omni_algos.get_omni_loss_fn("omni_sft")
        actor_config = _FakeActorConfig(omni_loss=OmniLossConfig(loss_mode="omni_sft", mse_weight=0.5))
        ...
        assert "omni_sft/ce_loss" in result.metrics
        assert "omni_sft/mse_loss" in result.metrics
