...
import verl_omni.trainer.diffusion.v1.trainer_base as trainer_base_v1
...
def _dump_v1(dump_path, outputs, *, max_samples=None, global_steps=0, audios=None, audio_sample_rates=None):
    """Invoke the static ``PolicyGradientDiffusionTrainerV1._write_generations``."""
    n = outputs.shape[0]
    inputs = [f"prompt {i}" for i in range(n)]
    gts = ["" for _ in range(n)]
    scores = [float(i) for i in range(n)]
    ...
    if audios is not None:
        kwargs = {"audios": audios, "audio_sample_rates": audio_sample_rates}
    PolicyGradientDiffusionTrainerV1._write_generations(
        inputs,
        outputs,
        gts,
        scores,
...
class TestDumpGenerationsV1:
    def test_v1_video_batch_writes_one_mp4_per_sample(self, tmp_path):
        outputs = torch.randint(256, (2, 8, 3, 16, 16), dtype=torch.uint8)  # [N, T, C, H, W]
        _dump_v1(tmp_path, outputs)
    ...
    def test_v1_video_batch_muxes_generated_audio(self, tmp_path):
        from imageio_ffmpeg import get_ffmpeg_exe

        outputs = torch.randint(256, (1, 8, 3, 16, 16), dtype=torch.uint8)
        audios = torch.sin(torch.linspace(0, 100, 48_000)).reshape(1, 1, -1)
        _dump_v1(tmp_path, outputs, audios=audios, audio_sample_rates=[48_000])

        path = os.path.join(str(tmp_path), "0", "0.mp4")
        probe = subprocess.run([get_ffmpeg_exe(), "-i", path], capture_output=True, text=True)
        assert "Video:" in probe.stderr and "Audio: aac" in probe.stderr

    def test_v1_video_export_failure_preserves_raw_fallback_and_writes_jsonl(self, monkeypatch, tmp_path):
        outputs = torch.randint(256, (2, 8, 3, 16, 16), dtype=torch.uint8)
        ...
        monkeypatch.setattr(trainer_base_v1, "_export_video", fake_export)
        _dump_v1(tmp_path, outputs)
    ...
    def test_v1_max_samples_bounds_video_dump(self, tmp_path):
        outputs = torch.randint(256, (3, 8, 3, 16, 16), dtype=torch.uint8)
        _dump_v1(tmp_path, outputs, max_samples=1)
