...
CONFIG_SPECS=(
    ...
    "fully_async_omni_megatron_trainer:_generated_fully_async_omni_megatron_trainer.yaml:--config-name=fully_async_omni_megatron_trainer.yaml"
)
...
for spec in "${CONFIG_SPECS[@]}"; do
    ...
    extra_arg=""
    if [ "$config_name" = "omni_trainer" ] || [ "$config_name" = "fully_async_omni_megatron_trainer" ]; then
        if [ -n "$VERL_CONFIG_DIR" ]; then
