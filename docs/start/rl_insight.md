...
Rollout engine statistics are exposed by each vLLM-Omni server and registered with RL-Insight. The generation path also emits a `vllm_generate` state trace for each rollout replica, using a separate lane such as `replica_0` or `replica_1`.
