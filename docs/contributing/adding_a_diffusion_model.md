...
| Prompt template | hard-coded `prompt_template_encode` with system role + `drop_idx=34` | user-only message via `apply_chat_template(..., add_generation_prompt=True, enable_thinking=True)`; no system prompt |
